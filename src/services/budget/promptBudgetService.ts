...
export class PromptBudgetService {
  ...
  validate(prompt: string, pricing?: LiveTokenPricing): PromptBudgetReport {
    ...
    if (!budget) {
      return { loaded: false, profile, violationCount: 0, violations: [] };
    }
  ...
  private check(profile: TokenProfileReport, budget: PromptBudgetFile): PromptBudgetViolation[] {
    ...
    if (typeof budget.maxTokens === "number" && profile.totalTokens > budget.maxTokens) {
      violations.push({ field: "maxTokens", message: `Prompt uses ${profile.totalTokens} tokens, above the ${budget.maxTokens} token budget.`, suggestedFix: "Open the optimization diff view and remove low-value context or repeated wording.", recommendedCommand: "promptguard.optimize" });
    }
    if (typeof budget.maxInputCostUsd === "number" && profile.estimatedInputCostUsd > budget.maxInputCostUsd) {
      violations.push({ field: "maxInputCostUsd", message: `Estimated input cost ${this.money(profile.estimatedInputCostUsd)} exceeds budget ${this.money(budget.maxInputCostUsd)}.`, suggestedFix: "Trim the prompt or move repetitive background into a reusable template.", recommendedCommand: "promptguard.optimize" });
    }
    if (typeof budget.maxOutputCostUsd === "number" && profile.estimatedOutputCostUsd > budget.maxOutputCostUsd) {
  ...
  private parse(source: string): PromptBudgetFile | undefined {
    try {
      ...
      return {
        ...
        maxInputCostUsd: typeof value.maxInputCostUsd === "number" ? value.maxInputCostUsd : undefined,
        maxOutputCostUsd: typeof value.maxOutputCostUsd === "number" ? value.maxOutputCostUsd : undefined,
        maxLatencyMs: typeof value.maxLatencyMs === "number" ? value.maxLatencyMs : undefined
      };
    } catch {
      return undefined;
    }
