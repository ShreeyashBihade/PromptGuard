...
export class PromptBenchmarkService {
  ...
  run(): PromptBenchmarkReport {
    ...
    if (!benchmarks) {
      return { loaded: false, suiteCount: 0, caseCount: 0, passedCount: 0, failedCount: 0, suites: [] };
    }
  ...
  private evaluateCase(suite: PromptBenchmarkSuite, testCase: PromptBenchmarkCase): PromptBenchmarkCaseResult {
    ...
    for (const criterion of testCase.criteria) {
      this.applyCriterionFailures(criterion, analysis.score.total, issues, tokenSavings, failures);
    }
  ...
  private applyCriterionFailures(criterion: PromptBenchmarkCriterion, score: number, issues: readonly PromptIssue[], tokenSavings: number, failures: string[]): void {
    if (typeof criterion.minScore === "number" && score < criterion.minScore) {
      failures.push(`${criterion.id}: score ${score} is below minimum ${criterion.minScore}.`);
    }
    if (typeof criterion.maxScore === "number" && score > criterion.maxScore) {
      failures.push(`${criterion.id}: score ${score} exceeds maximum ${criterion.maxScore}.`);
    }
    if (typeof criterion.maxIssueCount === "number" && issues.length > criterion.maxIssueCount) {
      failures.push(`${criterion.id}: issue count ${issues.length} exceeds maximum ${criterion.maxIssueCount}.`);
    }
    if (typeof criterion.minTokenSavings === "number" && tokenSavings < criterion.minTokenSavings) {
      failures.push(`${criterion.id}: token savings ${tokenSavings} is below minimum ${criterion.minTokenSavings}.`);
    }
    if (typeof criterion.maxTokenSavings === "number" && tokenSavings > criterion.maxTokenSavings) {
      failures.push(`${criterion.id}: token savings ${tokenSavings} exceeds maximum ${criterion.maxTokenSavings}.`);
    }

    const issueRuleIds = new Set(issues.map(issue => issue.ruleId));
    const issueCategories = new Set(issues.map(issue => issue.category));

    for (const ruleId of criterion.requiredRuleIds ?? []) {
      if (!issueRuleIds.has(ruleId)) {
        failures.push(`${criterion.id}: expected rule ${ruleId} to be triggered.`);
      }
    }
    for (const ruleId of criterion.forbiddenRuleIds ?? []) {
      if (issueRuleIds.has(ruleId)) {
        failures.push(`${criterion.id}: rule ${ruleId} should not be triggered.`);
      }
    }
    for (const category of criterion.requiredCategories ?? []) {
      if (!issueCategories.has(category)) {
        failures.push(`${criterion.id}: expected category ${category} to be present.`);
      }
    }
    for (const category of criterion.forbiddenCategories ?? []) {
      if (issueCategories.has(category)) {
        failures.push(`${criterion.id}: category ${category} should not be present.`);
      }
    }
  }
  ...
  private parse(source: string): PromptBenchmarkFile | undefined {
    try {
      ...
      const suites = value.suites.filter((suite): suite is PromptBenchmarkSuite => {
        return typeof suite === "object" && suite !== null && typeof (suite as { id?: unknown }).id === "string" && typeof (suite as { name?: unknown }).name === "string" && Array.isArray((suite as { cases?: unknown }).cases);
      }).map(suite => ({
        id: suite.id,
        name: suite.name,
        description: typeof suite.description === "string" ? suite.description : undefined,
        cases: suite.cases.filter((testCase): testCase is PromptBenchmarkCase => {
          return typeof testCase === "object" && testCase !== null && typeof (testCase as { id?: unknown }).id === "string" && typeof (testCase as { name?: unknown }).name === "string" && typeof (testCase as { prompt?: unknown }).prompt === "string" && Array.isArray((testCase as { criteria?: unknown }).criteria);
        }).map(testCase => ({
          id: testCase.id,
          name: testCase.name,
          ...
          criteria: testCase.criteria.filter((criterion): criterion is PromptBenchmarkCriterion => {
            return typeof criterion === "object" && criterion !== null && typeof (criterion as { id?: unknown }).id === "string" && typeof (criterion as { description?: unknown }).description === "string";
          }).map(criterion => ({
            id: criterion.id,
