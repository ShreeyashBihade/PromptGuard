...
import { PromptHistoryEntry, PromptOptimizationLedger } from "../../types";
import { PromptBudgetFile } from "../budget/promptBudgetService";
import { PromptBenchmarkReport } from "../benchmarks/promptBenchmarkService";
import { PromptLearningService, LearningSummary } from "../learning/promptLearningService";
import { PromptPolicyFile } from "../policy/promptPolicyService";
...
export class PromptAuditExportService {
  build(input: PromptAuditExportInput): PromptAuditExportReport {
    ...
    return {
      ...
      analytics,
      policyRuleCount: input.policy?.rules.length,
      budgetName: input.budget?.name,
      templateCount: input.templateCount,
      learningSignals: input.learning?.signalCount,
      benchmarkSummary: input.benchmarks ? {
        suiteCount: input.benchmarks.suiteCount,
        caseCount: input.benchmarks.caseCount,
        passedCount: input.benchmarks.passedCount,
        failedCount: input.benchmarks.failedCount,
        averageScore: input.benchmarks.averageScore
      } : undefined,
  ...
  renderMarkdown(report: PromptAuditExportReport): string {
    ...
    for (const [source, count] of Object.entries(report.analytics.sourceCounts)) {
      lines.push(`- ${source}: ${count}`);
    }
    if (!Object.keys(report.analytics.sourceCounts).length) {
      lines.push(`- No optimization entries yet.`);
    }
    lines.push(``);
    ...
    if (!report.recentEntries.length) {
      ...
      for (const entry of report.recentEntries) {
        lines.push(`- ${new Date(entry.timestamp).toLocaleString()} | score ${entry.score} | savings ${entry.estimatedSavings.toFixed(6)} | ${this.preview(entry.originalPrompt)}`);
      }
    }
    return `${lines.join("\n")}\n`;
  }
  ...
  private analytics(history: readonly PromptHistoryEntry[], ledger: PromptOptimizationLedger): PromptAuditTeamAnalytics {
    ...
    return {
      promptCount,
      averageScore,
      averageEstimatedSavings,
      totalReducedTokens: ledger.totals.totalReducedTokens,
      totalEstimatedSavingsUsd: ledger.totals.totalEstimatedSavingsUsd,
      sourceCounts
    };
  }
