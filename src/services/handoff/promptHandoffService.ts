...
export class PromptHandoffService {
  async export(workspaceRoot: string | undefined, artifact: PromptHandoffArtifact): Promise<PromptHandoffExportReport | undefined> {
    ...
    const enrichedArtifact: PromptHandoffArtifact = artifact.nextSteps ? artifact : {
      ...artifact,
      nextSteps: this.nextStepsForTarget(artifact.target)
    };
  ...
  renderHtml(artifact: PromptHandoffArtifact): string {
    ...
    const nextSteps = artifact.nextSteps?.length
      ? `<h2>Next steps</h2><ul>${artifact.nextSteps.map(step => `<li>${this.escapeHtml(step)}</li>`).join("")}</ul>`
      : "";
  ...
  private renderBridgeSnippet(target: "browser" | "jetbrains"): string {
    ...
    const snippet = target === "browser"
      ...
      : [
          "val artifact = jacksonObjectMapper().readValue<PromptHandoffArtifact>(jsonFile)",
          "val prompt = artifact.prompt",
          "clipboardManager.setContents(StringSelection(prompt))"
        ];
  ...
  private nextStepsForTarget(target: "browser" | "jetbrains"): readonly string[] {
    return target === "browser"
      ? [
          "Wrap PromptGuard analysis in a browser-side compose box or extension popup.",
          "Load the exported JSON artifact to prefill prompt text, source, and metadata.",
          "Call the local PromptGuard analyzer or bridge service before sending the prompt."
        ]
      : [
          "Wrap PromptGuard analysis in a JetBrains tool window or editor action.",
          "Load the exported JSON artifact to restore prompt text, source, and metadata.",
          "Call the local PromptGuard analyzer or bridge service before inserting the prompt."
        ];
  }
