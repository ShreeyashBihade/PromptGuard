...
export class TokenProfilerService {
  ...
  profile(input: TokenProfilerInput): TokenProfileReport {
    ...
    this.telemetry.emit("promptguard.tokenProfiler.profiled", {
      uri: input.uri ?? "",
      version: String(input.version ?? 0),
      ...
      latencyMs: String(report.latencyMs)
    });
  ...
  private profileNode(node: PromptAstNode, pricing: LiveTokenPricing, cache: { cacheHits(): void; cacheMisses(): void }): TokenProfileSection {
    ...
    const section: TokenProfileSection = {
      kind: node.kind,
      label: this.labelFor(node),
      text: node.text,
      tokenCount,
      importance: node.importance,
      ambiguityScore: node.ambiguityScore,
      duplicateScore: node.duplicateScore,
