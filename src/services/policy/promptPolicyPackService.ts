...
export class PromptPolicyPackService {
  ...
  list(): PromptPolicyPackReport {
    ...
    if (!file) {
      return { loaded: false, packCount: 0, enabledPackCount: 0, packs: [] };
    }

    const enabledPackCount = file.packs.filter(pack => pack.enabled !== false).length;
    return {
      source: this.policyPackPath(),
      loaded: true,
      packCount: file.packs.length,
      enabledPackCount,
      packs: file.packs
    };
  }
  ...
  private parse(source: string): PromptPolicyPackFile | undefined {
    try {
      ...
      const packs = value.packs.filter((pack): pack is PromptPolicyPack => {
        return typeof pack === "object" && pack !== null && typeof (pack as { id?: unknown }).id === "string" && typeof (pack as { name?: unknown }).name === "string" && Array.isArray((pack as { rules?: unknown }).rules);
      }).map(pack => ({
        id: pack.id,
        name: pack.name,
        ...
        rules: pack.rules.filter((rule): rule is PromptPolicyRule => {
