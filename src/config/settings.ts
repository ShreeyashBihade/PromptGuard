...
export const getSettings = (): PromptGuardSettings => {
  ...
  return { enabled: c.get("enabled", true), analyzeOnSave: c.get("analyzeOnSave", false), minimumPromptLength: c.get("minimumPromptLength", 20), disabledRules: c.get("disabledRules", []), ignoreComments: c.get("ignoreComments", false), modelPricing: c.get("modelPricing", []) };
};
