...
export class GroqGateway {
  ...
  async improveWithContext(prompt: string, context: string, findings: readonly string[], mode: "clarify" | "compress"): Promise<{ improvedPrompt: string; costUsd: number; outputTokens: number }> {
    const system = mode === "clarify"
      ? "You refine short prompts into compact, practical prompts. Return only the final prompt as plain text: no Markdown, bold markers, headings, bullet markers, tables, code blocks, examples, sample outputs, or commentary. Preserve the user's intent. Add only details explicitly supplied in the original prompt or clarification choices; do not invent deliverables, pages, features, standards, roles, constraints, target audiences, or output structure. Use short natural sentences or compact semicolon-separated phrases. The final prompt must be at most 130 words and should be as short as possible while remaining clear."
    ...
    const completionBudget = mode === "clarify" ? 220 : Math.min(4_000, Math.max(800, Math.ceil(prompt.length / 3)));
