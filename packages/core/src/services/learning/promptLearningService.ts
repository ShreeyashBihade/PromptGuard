...
export class PromptLearningService {
  ...
  recordAnalyze(result: AnalysisResult): void {
    this.append({ timestamp: result.analyzedAt, source: "analyze", score: result.score.total, issueCategories: result.issues.map(issue => issue.category), issueCount: result.issues.length, tokenSavings: result.optimization.estimatedTokenSavings ?? 0, timeSavedMs: 0, templateTags: [] });
  }
  ...
  recordOptimization(source: LearningSignal["source"], issues: readonly PromptIssue[], tokenSavings = 0, timeSavedMs = 0, outcome: LearningOptimizationOutcome = "accepted"): void {
    this.append({ timestamp: new Date().toISOString(), source, score: undefined, issueCategories: issues.map(issue => issue.category), issueCount: issues.length, tokenSavings, timeSavedMs, optimizationOutcome: outcome, templateTags: [] });
  }

  recordTemplate(tags: readonly string[]): void {
    this.append({ timestamp: new Date().toISOString(), source: "template", score: undefined, issueCategories: [], issueCount: 0, tokenSavings: 0, templateTags: tags });
  }

  summarize(): LearningSummary {
    const signals = this.load();
    if (!signals.length) return { loaded: false, signalCount: 0, acceptedOptimizationCount: 0, rejectedOptimizationCount: 0, issueCategories: {}, sourceCounts: {}, totalTokenSavings: 0, totalTimeSavedMs: 0 };
    ...
    for (const signal of signals) {
      ...
      for (const category of signal.issueCategories) issueCategories[category] = (issueCategories[category] ?? 0) + 1;
      if (signal.optimizationOutcome === "accepted") acceptedOptimizationCount += 1; else if (signal.optimizationOutcome === "rejected") rejectedOptimizationCount += 1;
      if (typeof signal.score === "number") { scoreTotal += signal.score; scoreCount += 1; }
      if (typeof signal.tokenSavings === "number") { savingsTotal += signal.tokenSavings; savingsCount += 1; }
      if (typeof signal.timeSavedMs === "number") { timeSavedTotal += signal.timeSavedMs; timeSavedCount += 1; }
    }
    return { source: this.learningPath(), loaded: true, signalCount: signals.length, acceptedOptimizationCount, rejectedOptimizationCount, issueCategories, sourceCounts, averageScore: scoreCount ? scoreTotal / scoreCount : undefined, averageTokenSavings: savingsCount ? savingsTotal / savingsCount : undefined, averageTimeSavedMs: timeSavedCount ? timeSavedTotal / timeSavedCount : undefined, totalTokenSavings: savingsTotal, totalTimeSavedMs: timeSavedTotal };
  }
  ...
  private parseSignals(value: unknown): LearningSignal[] {
    ...
    return value.filter((signal): signal is LearningSignal => typeof signal === "object" && signal !== null && typeof (signal as { timestamp?: unknown }).timestamp === "string" && typeof (signal as { source?: unknown }).source === "string").slice(0, 500);
  }
