...
export class PromptProviderRegistryService {
  ...
  list(): PromptProviderRegistryReport {
    ...
    if (!catalog) return { loaded: false, providers: [] };
    return { source: this.catalogPath(), loaded: true, providers: catalog.providers.map(provider => ({ id: provider.id, enabled: provider.enabled ?? provider.id === "groq", preferredModels: provider.preferredModels ?? [] })) };
  }
  ...
  async setEnabled(id: PromptProviderId, enabled: boolean, preferredModels: readonly string[] = []): Promise<PromptProviderRegistryReport> {
    ...
    if (!catalogPath) throw new Error("Open a workspace folder to edit provider opt-in settings.");
  ...
  private load(): PromptProviderCatalogFile | undefined {
    ...
    try {
      ...
      if (parsed.version !== 1 || !Array.isArray(parsed.providers)) return undefined;
