...
export class PromptPolicyService {
  ...
  validate(prompt: string, analysis?: AnalysisResult): PromptPolicyReport {
    ...
    if (!policy) return { loaded: false, ruleCount: 0, violations: [] };
    const violations = [...this.validateSettings(prompt, policy, analysis), ...policy.rules.flatMap(rule => this.validateRule(prompt, rule))];
  ...
  private validateRule(prompt: string, rule: PromptPolicyRule): PromptPolicyViolation[] {
    ...
    if (typeof rule.minLength === "number" && prompt.length < rule.minLength) violations.push(this.violation(rule, `Prompt must be at least ${rule.minLength} characters.`));
    if (typeof rule.maxLength === "number" && prompt.length > rule.maxLength) violations.push(this.violation(rule, `Prompt must be at most ${rule.maxLength} characters.`));
    if (typeof rule.pattern === "string" && rule.pattern.trim()) {
      try {
        const expression = new RegExp(rule.pattern, "i");
        if (!expression.test(prompt)) violations.push(this.violation(rule, `Prompt must match pattern /${rule.pattern}/.`));
      } catch {
        violations.push(this.violation(rule, `Invalid policy pattern /${rule.pattern}/.`));
      }
    }
    for (const term of rule.requiredTerms ?? []) {
      if (!normalized.includes(term.toLowerCase())) violations.push(this.violation(rule, `Prompt must include "${term}".`));
    }
    for (const term of rule.forbiddenTerms ?? []) {
      if (normalized.includes(term.toLowerCase())) violations.push(this.violation(rule, `Prompt must not include "${term}".`));
    }
  ...
  private validateSettings(prompt: string, policy: PromptPolicyFile, analysis?: AnalysisResult): PromptPolicyViolation[] {
    ...
    if (policy.requireOutput && !/\b(output|format|return|respond|json|markdown|table|bullets?)\b/i.test(prompt)) {
    ...
    if (policy.forbidSecrets && /(api[_-]?key|api\s+key|secret|token|password|passwd|private key|ssh-rsa|bearer\s+[a-z0-9._-]+)/i.test(prompt)) {
    ...
    if (policy.requireConstraints && !/(must|must not|avoid|limit|only|exactly|at most|under|include|exclude|constraints?|requirements?)/i.test(prompt)) {
  ...
  private parse(source: string): PromptPolicyFile | undefined {
    try {
      ...
      const rules = Array.isArray(value.rules)
        ? value.rules.filter((rule): rule is PromptPolicyRule => typeof rule === "object" && rule !== null && typeof (rule as { id?: unknown }).id === "string" && typeof (rule as { description?: unknown }).description === "string")
        : [];
      if (value.version !== undefined && value.version !== 1) return undefined;
      if (!rules.length && !settings.maxTokens && !settings.requireOutput && !settings.forbidSecrets && !settings.requireConstraints) return undefined;
