...
export class LocalPromptAdvisor {
  build(prompt: string, mode: LocalInsights["mode"], learning?: LearningSummary): LocalInsights {
    ...
    return { mode, bestPractices: this.bestPractices(prompt, task, learning), recommendations: this.recommend(task, learning) };
  }
  ...
  private bestPractices(prompt: string, task: ReturnType<LocalPromptAdvisor["detectTask"]>, learning?: LearningSummary): string[] {
    const practices = ["State the output format explicitly (table, JSON, bullet list, or checklist).", "Add acceptance criteria so the model can self-check before finalizing.", "Include constraints (token budget, style, must-include and must-avoid items)."];
    if (learning?.loaded) {
      if (learning.acceptedOptimizationCount > learning.rejectedOptimizationCount) practices.unshift("You usually accept concise rewrites, so keep the core task short and add guardrails only where they help.");
      else if (learning.rejectedOptimizationCount > learning.acceptedOptimizationCount) practices.unshift("You often reject aggressive compression, so preserve examples and constraints when suggesting edits.");
      ...
      if (topCategories.includes("constraints")) practices.push("Lead with explicit constraints because your accepted optimizations often depend on tighter boundaries.");
      if (topCategories.includes("formatting")) practices.push("Keep output format instructions explicit and stable across revisions.");
    }
  ...
  private recommend(task: ReturnType<LocalPromptAdvisor["detectTask"]>, learning?: LearningSummary): ModelRecommendation[] {
    const ranked = MODEL_CATALOG.map(profile => {
      const score = profile.strengths.includes(task === "long-context" ? "long context" : task) ? 3 : profile.strengths.includes("analysis") && task === "analysis" ? 2 : profile.strengths.includes("general-purpose") ? 1 : 0;
      return { profile, score };
    }).sort((a, b) => b.score - a.score).filter(item => item.score > 0);
    ...
    for (const item of ranked) {
      ...
      picks.push({ provider: item.profile.provider, model: item.profile.model, fit: item.score >= 3 ? "high" : "medium", rationale: `Strong for ${task.replace("-", " ")} workloads; tradeoff: ${item.profile.weakness}.` });
    ...
    if (learning?.loaded && learning.acceptedOptimizationCount > 0 && picks[0]) {
      ...
      picks[0] = { ...picks[0], rationale: `${picks[0].rationale} Your accepted optimizations average ${tokenSavings.toFixed(1)} saved tokens and ${Math.round(timeSavedMs)}ms saved time.` };
    }
